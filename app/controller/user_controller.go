...
func (c *userController) SaveUser(ctx *gin.Context) {
	...
	if userAPI.Password == nil {
		userAPI.Password = lib.Strptr(lib.GeneratePassword(16, 4, 4, 4))
		isPasswordNull = true
	}
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, lib.BuildErrorResponse("role not found", err.Error(), nil))
		return
	}

	// Encrypt Password
	cipherEncryptSalt, _ := lib.CipherEncrypt(os.Getenv("SALT"), os.Getenv("CIPHER_KEY"))
	cipherEncrypt, _ := lib.CipherEncrypt(string(cipherEncryptSalt)+"||"+*user.Password, os.Getenv("CIPHER_KEY"))
	user.Password = lib.Strptr(base64.StdEncoding.EncodeToString(cipherEncrypt))

	_, err = c.userService.Create(user)
	if err != nil {
		ctx.JSON(http.StatusConflict, lib.BuildErrorResponse("failed to create user", err.Error(), user))
		return
	}
	...
	if fileHeader.Filename != "" {
		...
		if err != nil {
			...
			ctx.JSON(http.StatusInternalServerError, lib.BuildErrorResponse("failed upload image to google drive", "user failed to created", user))
			return
		}
		...
		if err != nil {
			ctx.JSON(http.StatusInternalServerError, lib.BuildErrorResponse("failed update image data", err.Error(), user))
			return
		}
	}
	if isPasswordNull {
	...
	ctx.JSON(http.StatusOK, lib.BuildResponse(true, "success", user))
}
...
func (c *userController) GetUsers(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, lib.BuildErrorResponse("users not found", err.Error(), nil))
		return
	}
	...
	if userCheck != nil {
		if *userCheck.Role.Name == utils.RoleNameAdmin {
			ctx.JSON(http.StatusOK, lib.BuildResponse(true, "success", users))
			return
		}
	} else {
		_ = lib.Merge(users, &usersType)
		ctx.JSON(http.StatusOK, lib.BuildResponse(true, "success", usersType.PublicUsers()))
		return
	}
}
...
func (c *userController) GetUser(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, lib.BuildErrorResponse("user not found", err.Error(), user))
		return
	}
	userCheck, _ := utils.AdminAuthMiddleware(c.auth, c.redis, c.userService, c.roleService, ctx, "admin")
	if userCheck != nil {
		...
		ctx.JSON(http.StatusOK, lib.BuildResponse(true, "success", user.PublicUser()))
		return
	}
...
func (c *userController) GetUsersByRoleId(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, lib.BuildErrorResponse("role id must number", err.Error(), nil))
		return
	}
	limit, offset := utils.GetLimitOffsetParam(ctx)
	users, err := c.userService.FindAllByRoleId(int64(id), offset, limit)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, lib.BuildErrorResponse(fmt.Sprintf("users with role id %d not found", id), err.Error(), nil))
		return
	}
	userCheck, _ := utils.AdminAuthMiddleware(c.auth, c.redis, c.userService, c.roleService, ctx, "admin")
	if userCheck != nil {
...
func (c *userController) GetUserByUsername(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusBadRequest, lib.BuildErrorResponse(fmt.Sprintf("user with username %s not found", username), err.Error(), nil))
		return
	}
	userCheck, _ := utils.AdminAuthMiddleware(c.auth, c.redis, c.userService, c.roleService, ctx, "admin")
	if userCheck != nil {
		if *userCheck.Role.Name == utils.RoleNameAdmin {
			ctx.JSON(http.StatusOK, lib.BuildResponse(true, "success", findUserByUsername))
			return
		}
	} else {
...
func (c *userController) UpdateUser(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusUnauthorized, lib.BuildErrorResponse("unauthorized", err.Error(), checkIdUser))
		return
	}
	if *checkIdUser.ID != uuidParam && *checkIdUser.RoleId != 1 {
		ctx.JSON(http.StatusUnauthorized, lib.BuildErrorResponse("can't update data, your id not equals", err.Error(), checkIdUser))
		return
	}
	...
	if err != nil {
		ctx.JSON(http.StatusUnprocessableEntity, lib.BuildErrorResponse("can't update user", err.Error(), user))
		return
	}
	fileHeader, _ := ctx.FormFile("images")
...
func (c *userController) DeleteUser(ctx *gin.Context) {
	...
	if *checkIdUser.ID != uuidParam && *checkIdUser.RoleId != 1 {
		...
		if err != nil {
			ctx.JSON(http.StatusBadRequest, lib.BuildErrorResponse("id not found", err.Error(), uuidParam))
			return
		}
		ctx.JSON(http.StatusOK, lib.BuildResponse(true, "success", nil))
	}
}
...
func (c *userController) CountUsers(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, lib.BuildErrorResponse("failed to count user", err.Error(), nil))
		return
	}
