...
func (s *Repositories) Seeder() error {
	...
	result, err = s.db.Exec("CREATE TABLE IF NOT EXISTS users (uuid uuid PRIMARY KEY, first_name VARCHAR(55) not null, last_name VARCHAR(55) not null, email VARCHAR(55) unique not null, " +
		"phone_number VARCHAR(15) not null, username VARCHAR(55) unique not null, password VARCHAR(255) not null, image VARCHAR(255), role_id int not null, created_at timestamp, updated_at timestamp, deleted_at timestamp)")
	...
	// log.Println(result, err)
	result, err = s.db.Exec("CREATE TABLE IF NOT EXISTS posts (id serial PRIMARY KEY, title VARCHAR(100) not null unique, description text not null, post_images text, user_uuid uuid not null, " +
		"created_at timestamp, updated_at timestamp, deleted_at timestamp)")
	// log.Println(result, err)
